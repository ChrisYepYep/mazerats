...
const PRIVACY_SECTIONS = [
    ...
    },
    {
        ...
        body: "Every round of Fallin' Furni is recorded so the game can be tuned - which levels people clear, which they run out of time on, and where a run ends. A record holds what happened in the game: the levels played, how long each took, how many seats were taken, how the round ended, the points scored, the size of the browser window, and whether the screen was a touchscreen. If you are signed in it carries your Discord display name and ID, the same identity the leaderboard already shows. It also holds the IP address the round was played from, whether or not you are signed in, and we group runs by that address so that we can see when one leaderboard is being played by several accounts from the same connection. That means a run played without signing in is not anonymous: it can be grouped with other runs from the same address, including ones where somebody did sign in. This is the only part of the site that keeps an IP address against what you did rather than for short-term spam and security filtering, and it applies to Fallin' Furni alone - the interaction records described below still have no address on them. These records are never shown publicly and are only ever read by an administrator looking at how the game is playing; they are deleted automatically after 180 days, address and all."
    },
    {
